  ...
  <div class="lg:container px-[1rem] pt-6 flex gap-16">
    ...
    <div class="flex flex-col gap-6 text-white">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex lg:gap-16 gap-8" v-if="props.show.release_date">
          <span class="w-24">Released</span>
          <p>{{ props.show.release_date }}</p>
        </div>
        <div class="flex lg:gap-16 gap-8" v-if="props.show.runtime">
          <span class="w-24">Runtime</span>
          <p>{{ props.show.runtime }} min</p>
        </div>
        <!-- <div class="flex lg:gap-16 gap-8" v-if="props.show.episode_run_time.length != 0">
          <span class="w-24">Runtime</span>
          <p>{{ props.show.episode_run_time[0] }}</p>
        </div> -->
        <div class="flex lg:gap-16 gap-8" v-if="props.show.budget">
          <span class="w-24">Budget</span>
          <p>${{ props.show.budget }}</p>
        </div>
        <div class="flex lg:gap-16 gap-8" v-if="props.show.revenue">
          <span class="w-24">Revenue</span>
          <p>${{ props.show.revenue }}</p>
        </div>
        <!-- add page for the route -->
        <div class="flex lg:gap-16 gap-8" v-if="props.show.genres">
          <span class="w-24">Genre</span>
          <div class="flex gap-2">
            <a v-for="genre in props.show.genres" :key="genre.id" class="text-[#1E89DE] underline">
              {{ genre.name }}
            </a>
          </div>
        </div>
        ...
        <div class="flex lg:gap-16 gap-8" v-if="props.show.spoken_languages">
          <span class="w-24">Language</span>
          <div class="flex gap-2">
            <p v-for="lang in props.show.spoken_languages" :key="lang.id">
              {{ lang.english_name }}
            </p>
          </div>
        </div>
        <div class="flex lg:gap-16 gap-8" v-if="props.show.production_companies">
          <span class="w-24">Production</span>
          <div class="flex gap-2">
            <p v-for="prod in props.show.production_companies" :key="prod.id">
              {{ prod.name }}
            </p>
          </div>
        </div>
