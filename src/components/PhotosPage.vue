...
onMounted(() => {
  if (route.name == 'tvPhotos') {
    getData(route.params.id, 'tv')
  } else getData(route.params.id, 'movie')
})
</script>
<template>
  <div class="lg:px-[3rem] px-[1rem] flex flex-col gap-12" v-motion-fade>
    ...
    <div class="flex flex-col gap-6" v-if="hasImage">
    ...
    <div class="flex justify-center items-center h-36" v-else-if="hasImage == false">
